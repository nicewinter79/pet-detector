...
from zone_detector import process_video, ZONES
...
def render_video_upload_section(model, conf: float):
    st.subheader("Process a Video")
    st.write(
        "Upload a video to detect zone entries/exits. This runs the same "
        "logic as `zone_detector.py` and adds the results to the events "
        "below. Processing runs on CPU here, so it will be slower than a "
        "local GPU run -- start with a short clip."
    )
    ...
    if uploaded_video is not None:
        if st.button("Process Video"):
            ...
            def update_progress(current_frame, total_frames):
                # total_frames isn't always known ahead of time with stream=True,
                # so just show a moving indicator based on frame count so far.
                progress_bar.progress(min(current_frame / 500, 1.0), text=f"Processed {current_frame} frames...")
            ...
            with st.spinner("Processing video (this may take a while on CPU)..."):
                summary = process_video(
                    ...
                    zones=ZONES,
                    conf=conf,
                    out_path="zone_output.mp4",
                    log_path=ZONE_LOG_PATH,
                    progress_callback=update_progress,
                )

            progress_bar.empty()
            st.success(f"Done! Processed {summary['frame_count']} frames.")
            if summary["event_counts"]:
                st.write("Events detected in this video:")
                for (class_name, event, zone_id), count in sorted(summary["event_counts"].items()):
                    st.write(f"- {class_name} {event} {zone_id}: {count}")
            else:
                st.info("No zone crossings detected in this video.")

            st.rerun()


def render_zone_events_tab(model, conf: float):
    render_video_upload_section(model, conf)
    ...
    if events.empty:
        st.info("No zone events found yet. Upload and process a video above, or run `zone_detector.py --source your_video.mp4` locally.")
        return
...
def main():
    ...
    with tab_zone:
        render_zone_events_tab(model, conf)
